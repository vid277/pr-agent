...
class DICLImprovementTester:
    ...
    async def test_dicl_improvement(self, test_subset_size: int = 15) -> Dict[str, Any]:
        ...
        improvement_analysis = self._analyze_improvement(
            baseline_report, enhanced_report
        )
...
Example: "Line 15: CRITICAL - privileged: true allows container escape to host system"""
        ...
        pr_data["pr_id"] = f"eval_{test_case.id}"
        pr_data["diff"] = test_case.code_content
    ...
    def _analyze_improvement(self, baseline_report, enhanced_report) -> Dict[str, Any]:
        f1_improvement = (
            enhanced_report.overall_f1_score - baseline_report.overall_f1_score
        )
        precision_improvement = (
            enhanced_report.overall_precision - baseline_report.overall_precision
        )
        recall_improvement = (
            enhanced_report.overall_recall - baseline_report.overall_recall
        )
        ...
        return {
            ...
            "improvement_percentage": (
                ...
                if baseline_report.overall_f1_score > 0
                else 0
            ),
            "conclusion": self._generate_conclusion(
                f1_improvement, significant_threshold
            ),
        }
...
async def main():
    ...
    print(f"Precision Impovement: {analysis['improvements']['precision']:+.3f}")
    print()
