...
class PRReviewer:
    ...
    def __init__(
        ...
        self.vars = {
            ...
            "require_todo_scan": get_settings().pr_reviewer.get(
    ...
    async def _get_prediction(self, model: str) -> str:
        ...
        pr_data = {
            ...
            "changed_files": self.git_provider.get_files(),
            "language": variables.get("language", ""),
        }
        original_prompt_length = len(user_prompt)
        user_prompt = await DICL.regular(pr_data, user_prompt)
        enhanced_prompt_length = len(user_prompt)
        get_logger().info(
            f"DICL enhanced prompt: +{enhanced_prompt_length - original_prompt_length} chars"
        )
    ...
    def _prepare_pr_review(self) -> str:
        ...
        if "review" not in data:
            get_logger().exception(
                "Failed to parse review data", artifact={"data": data}
            )
            return ""
